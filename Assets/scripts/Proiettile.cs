...
public class Proiettile : MonoBehaviour //CLASSE PER GESTIRE L'OGGETTO PROIETTILE
{
    ...
    public void shoot_bullet() //mitraglietta classica
    {
        ...
        proiettile_clone = Instantiate(proiettile, triangle.transform.position + triangle.partition_vect(triangle.direction) * bullet_spawn_vert, triangle.transform.rotation); //creo il nuovo oggetto proiettile(con un offsett rispetto alla prua della nave)
        Vector3 bullet_direction = triangle.partition_vect(triangle.direction); //ottengo il vettore della direzione del proiettile(uso la funz. partition_vect per ottenere il vettore della direzione))
