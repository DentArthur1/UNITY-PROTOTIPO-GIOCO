...
public class DirectionIndicator : MonoBehaviour //CLASSE PER GESTIRE IL MOVIMENTO DELLA FRECCIA
{
    ...
    void scale_arrow() { //scala la grandezza della freccia in modo che sia costante ai cambiamenti di zoom
        ...
        engine.transform.localScale = new Vector3(scale_x * mult_factor, scale_y * mult_factor, engine.transform.localScale.z);
        thruster.transform.localScale = new Vector3(scale_x * mult_factor, scale_y * mult_factor, thruster.transform.localScale.z);
        expected.transform.localScale = new Vector3(scale_x * mult_factor, scale_y * mult_factor, expected.transform.localScale.z);
    }
